...
/datum/controller/subsystem/shuttle/is_in_shuttle_bounds(atom/A)
...
/datum/controller/subsystem/shuttle/get_containing_shuttle(atom/A)
	var/area/param_area = get_area(A)
	if(istype(param_area, /area/ship))
...
/datum/controller/subsystem/shuttle/get_dock_overlap(x0, y0, x1, y1, z)
	. = list()
	for(var/obj/docking_port/port as anything in stationary)
		if(!port || port.z != z)
			continue
 ...
 **/
/datum/controller/subsystem/shuttle/load_template(datum/map_template/shuttle/template, datum/overmap/ship/controlled/parent, spawn_transit = TRUE)
	. = FALSE
