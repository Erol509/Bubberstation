...
/obj/machinery/fusion_fuel_injector/on_update_icon()
	...
	if(injecting && cur_assembly)
		add_overlay(emissive_appearance(icon, "[icon_state]_lights"))
...
/obj/machinery/fusion_fuel_injector/use_tool(obj/item/W, mob/living/user, list/click_params)
	...
	if(istype(W, /obj/item/fuel_assembly))
		...
		if(!user.unEquip(W, src))
			return TRUE
		if(cur_assembly)
			visible_message(span_warning("\The [user] swaps \the [src]'s [cur_assembly] for \a [W]."))
		else
			visible_message(span_warning("\The [user] inserts \a [W] into \the [src]."))
		if(cur_assembly)
			cur_assembly.dropInto(loc)
...
/obj/machinery/fusion_fuel_injector/proc/Inject()
	...
	if(cur_assembly)
		...
		add_overlay(emissive_appearance(icon, "[icon_state]_lights_emitting"))
		add_overlay("[icon_state]_lights_emitting")
	else
		StopInjecting()
		update_icon()
