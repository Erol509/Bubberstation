...
/obj/effect/fusion_em_field/process()
	...
	Radiate()
	if(radiation)
		SSradiation.irradiate(src, round(radiation*0.001))
	return 1
...
/obj/effect/fusion_em_field/proc/warning()
	...
	if(percent_unstable > 0.5)
		...
		alert_msg = null
	if(alert_msg)
		radio.talk_into(alert_msg, "INDRA Reactor Monitor", "Engineering")

		if((percent_unstable > 0.9) && !public_alert)
			alert_msg = null
			radio.talk_into(emergency_alert, "INDRA Reactor Monitor")
			public_alert = TRUE
			for(var/mob/M in GLOB.player_list)
				var/turf/T = get_turf(M)
				if(T && !istype(M, /mob/abstract/new_player) && !isdeaf(M))
					alert_sound_to_playing(M, 'sound/effects/nuclearsiren.ogg')
		else if(safe_warned && public_alert)
			radio.talk_into(alert_msg, "INDRA Reactor Monitor")
			public_alert = FALSE
