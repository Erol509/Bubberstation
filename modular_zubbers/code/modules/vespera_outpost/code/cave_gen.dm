/datum/map_generator/cave_generator/vespera_forest
	weighted_open_turf_types = list(/turf/open/misc/dirt/jungle = 19, /turf/open/misc/dirt/jungle/dark = 4, /turf/open/misc/grass/jungle = 8)
	weighted_closed_turf_types = list(/turf/closed/mineral/random/jungle = 5)
	weighted_mob_spawn_list = list(
		/mob/living/basic/butterfly = 1,
		/mob/living/basic/bat = 2
	) // Jungle mobs are.. a bit too on the deadcode and strong as hell side to just have spawning.

	weighted_flora_spawn_list = list(
		/obj/structure/flora/grass/jungle/a/style_random = 8,
		/obj/structure/flora/grass/jungle/b/style_random = 12,
		/obj/structure/flora/bush/jungle/a/style_random = 5,
		/obj/structure/flora/bush/jungle/b/style_random = 2,
		/obj/structure/flora/bush/jungle/c/style_random = 8,
		/obj/structure/flora/bush/large/style_random = 2,
		/obj/structure/flora/rock/pile/jungle/style_random = 5,
	)

	mob_spawn_chance = 1
	smoothing_iterations = 50
